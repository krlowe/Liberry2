...
class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, ItemSelectionViewControllerDelegate {
	...
	override func viewDidLoad() {
        ...
        // Set up some defaults from our saved settings
        let defaults = UserDefaults.standard
        if (defaults.object(forKey: "name") as? String) != nil {
            playButton.setTitle("✅", for: .normal)
            playButtonIs = .check
            destinationLabel.text = "{Scan destination or tap to check out}"
            titleLabel.text = ""
            errorLabel.text = ""
        }
    ...
    @IBAction func changeDestination(_ sender: UIButton) {
        // Set up some defaults from our saved settings
        let defaults = UserDefaults.standard
        ...
        case .pause:
            ...
            if name != nil {
                playButton.setTitle("✅", for: .normal)
                playButtonIs = .check
                destinationLabel.text = "{Scan location or tap to check out}"
            } else {
	...
	*/
	...
	private func createMetadataObjectOverlayWithMetadataObject(_ metadataObject: AVMetadataObject) -> MetadataObjectLayer {
		...
		metadataObjectOverlayLayer.lineJoin = CAShapeLayerLineJoin.round
		...
		if let barcodeMetadataObject = transformedMetadataObject as? AVMetadataMachineReadableCodeObject {
			...
			if let stringValue = barcodeMetadataObject.stringValue, !stringValue.isEmpty {
                if stringValue != lastScan {
                    UIApplication.shared.isIdleTimerDisabled = true // weke up our display
                    ...
                    if tokens.count != 2 {
                        ...
                        let verb = String(tokens[1])
                        if verb[verb.startIndex] == "i" {
                            if lastDest == "" {
                                errorLabel.text = "Select location before scanning items"
                            } else {
                                errorLabel.text = ""
                                url = stringValue + "&" + lastDest
                            }
                        } else if verb[verb.startIndex] == "p" {
                            errorLabel.text = ""
                            url = stringValue
                        } else {
                            errorLabel.text = "Unrecoginized scanned code"
                        }
                    }
                    ...
                    if url != "" {
                        ...
                        if let request = URL(string: url) {
                            let task = session.dataTask(with: request, completionHandler: { data, response, error in
                                ...
                                do {
                                    ...
                                    if let pcode = json["pcode"] as? String {
                                        self.lastDest = pcode
                                        self.titleLabel.text = ""
                                        self.errorLabel.text = ""
                                        if let name = json["location"] as? String {
                                            ...
                                            if (json["default"] as? Bool) != nil {
                                                ...
                                                self.playButtonIs = .pause
                                            } else {
                                                if let _ = defaults.object(forKey: "name") as? String {
                                                    self.playButton.setTitle("✅", for: .normal)
                                                    self.playButtonIs = .check
                                                } else {
                                                    self.playButton.setTitle("⏸", for: .normal)
                                                    self.playButtonIs = .pause
                                                }
                                            ...
                                            self.lastDest = ""
                                            self.destinationLabel.text = "{Rescan location}"
                                            self.playButton.setTitle("⏸", for: .normal)
                                            self.playButtonIs = .pause
                                            self.errorLabel.text = "Unknown location name for \(pcode)"
                                        }
                                        return
                                    }
                                    
                                    self.titleLabel.text = ""
                                    self.errorLabel.text = "Expected keywords not found in response"
                                    print(json)
                                    }
                                } catch {
                                    print("JSON error: \(error.localizedDescription)")
                                    self.errorLabel.text = "JSON error: \(error.localizedDescription)"
                                }
                            })
                            task.resume()
                        }
                        ...
                        Alamofire.request(url).responseJSON { response in
                            ...
                            case .failure(let error):
                                print("Query \(stringValue) failed: \(error)")
                                self.errorLabel.text = "Query failed: \(error)"
                                return
                                
                            case .success(let data):
                                // First make sure we got back a dictionary
                                guard let json = data as? [String : AnyObject] else {
                                    self.errorLabel.text = "Failed to get expected response from query"
                                    return
                                }
